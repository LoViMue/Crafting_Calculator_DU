...
Crafting_List = []
myapp = tk.Tk()
myapp.title("Louis's_Crafting_Calulator")
#def inputText():


was0 = tk.Entry(myapp, width=15)
was0.grid(row=1, column=1, sticky="w" )
...
def ADD():
    ...
    try:
        ...
        error = tk.Label(myapp, text="1.Select the thing 2.Ammount in 3. +", fg="red")
        error.grid(row=0, column=2, padx=3, sticky="e")
...
def update_crafting_list():
    ...
    for i, was in enumerate(Crafting_List):
        ...
        if i + 1 == len(Crafting_List):
            ent.append([tk.Button(myapp, text="del", command=remove_crafting_list_item), tk.Button(myapp, text="Calculate", command=Calculate)])
...
#padx,pady,DISABLE, command=, fg,bg
